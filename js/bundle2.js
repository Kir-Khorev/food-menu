...
function calc() {
  ...
  var age, sex, weight, height, ratio; // Проверяем значения в Local Storage
  ...
  initLocalSettings('.calculating__choose_big div', 'calculating__choose-item_active'); // Получаем статичные данные
  ...
  getStaticInformation('.calculating__choose_big div', 'calculating__choose-item_active'); // Получаем динамические данные
  ...
  getDynamicInformation('#age'); // Считаем по формуле необходимые каллории
...
function forms(formSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    ...
    form.addEventListener('submit', function (e) {
      // Навешиваем событие подписки
      e.preventDefault(); // Отменяем стандартное действие события подписки.

      var statusMessage = document.createElement('img'); // Создаём сообщение на странице

      statusMessage.src = message.loading;
      statusMessage.style.cssText = "\n                    display: block;\n                    margin: 0 auto;\n                ";
      form.insertAdjacentElement('afterend', statusMessage); // request.setRequestHeader('Content-type', 'application/json; charset=utf-8'); // Когда используем связку XMLHttpReq и formData - Заголовок устанавливать не нужно.

      var formData = new FormData(form); // Создание новой формы (в параметре указана изначальная форма)
...
function slider(_ref) {
  ...
  function changeDots() {
    ...
    dots[slideIndex - 1].style.opacity = '1.5';
  }
...
function timer(id, deadline) {
  // TIMER
  function getTimeRemainin(endtime) {
    var t = Date.parse(endtime) - Date.parse(new Date()); // console.log(t);
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.next = 2;
            return fetch(url, {
              // Кода сначала выполняет правую часть (после равно). Поэтому нужен async/await.
              method: 'POST',
              headers: {
